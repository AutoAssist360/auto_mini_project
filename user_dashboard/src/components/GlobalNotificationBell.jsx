...
import { subscribeToNotificationUnreadSync } from '../lib/notificationEvents'
...
export default function GlobalNotificationBell() {
  ...
  useEffect(() => subscribeToNotificationUnreadSync(({ unreadCount, unreadDelta }) => {
    ...
    if (typeof unreadDelta === 'number') {
      setUnread((current) => Math.max(0, current + unreadDelta))
    }
  }), [])
  ...
  return (
    <Link
      ...
      className="fixed right-3 top-3 z-[70] flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 bg-white/90 text-slate-700 shadow-xl backdrop-blur transition hover:scale-105 hover:border-blue-500 hover:text-blue-600 dark:border-slate-700 dark:bg-slate-900/90 dark:text-slate-100 dark:hover:border-blue-400 dark:hover:text-blue-400 sm:right-6 sm:top-6 sm:h-12 sm:w-12"
    >
      ...
      {unread > 0 && (
        <span className="absolute -right-1 -top-1 flex min-h-5 min-w-5 items-center justify-center whitespace-nowrap rounded-full bg-red-500 px-1 text-[10px] font-black text-white ring-2 ring-white dark:ring-slate-900">
          {unread > 99 ? '99+' : unread}
        </span>
      )}
    </Link>
