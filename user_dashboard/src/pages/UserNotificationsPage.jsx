  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#030712] text-slate-900 dark:text-slate-100 font-['Outfit',_sans-serif] transition-colors duration-500">
      ...
      <div className="relative mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        {/* Sticky Header */}
        <header className="sticky top-4 z-40 mb-8 rounded-full border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-[#0B1120]/80 backdrop-blur-md px-6 py-3 shadow-xl flex items-center justify-between transition-all">
          ...
          <div className="flex items-center gap-2">
             <MobileNav>
                <div className="flex items-center gap-2 pr-2 border-r border-slate-200 dark:border-slate-800 mr-2">
                   <button onClick={onToggleTheme} className="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-xs transition-all">{theme === 'dark' ? '🌞' : '🌙'}</button>
                </div>
                {items.some((n) => !n.is_read) && (
                  <button onClick={handleMarkAll} className="px-4 py-2 rounded-2xl bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[9px] font-black uppercase tracking-widest border border-blue-500/20 hover:bg-blue-500 hover:text-white transition-all">
                    Mark All Read
                  </button>
                )}
                <button onClick={handleLogout} className="px-5 py-2.5 rounded-2xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[9px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-lg ml-1">
                   LOGOUT
                </button>
             </MobileNav>
          </div>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-4 pt-8">
              <button 
                disabled={page <= 1} 
                onClick={() => load(page - 1)}
                className="h-10 px-6 rounded-2xl border border-slate-200 dark:border-slate-800 text-[10px] font-black uppercase tracking-widest disabled:opacity-40 hover:border-blue-500 transition-all flex items-center gap-2 bg-white dark:bg-[#0B1120]"
              >
              ...
              <div className="h-10 px-4 rounded-2xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[10px] font-black uppercase flex items-center tracking-tighter shadow-lg">
