...
// helper schema that accepts either a full ISO datetime or a plain YYYY-MM-DD date
const dateOrDay = z.preprocess((val) => {
  if (typeof val === 'string') {
    // if user supplies date-only, append time so that the datetime schema accepts it
    if (/^\d{4}-\d{2}-\d{2}$/.test(val)) {
      // use UTC midnight; downstream code may adjust to end of day
...
export const analyticsQuery = z.object({
  // note: original client code passes `YYYY-MM-DD` from type=date inputs
  // the preprocess step above converts that into a valid datetime string
  from: dateOrDay.optional(),
  to: dateOrDay.optional(),
